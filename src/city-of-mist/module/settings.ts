...
export class CitySettings {
    static async set<K extends keyof SettingsType>(
        settingField: K,
        newvalue: SettingsChoices<K>,
    ) {
        await game.settings.set("city-of-mist", settingField, newvalue);
    }

    static get<K extends keyof SettingsType>(
        settingName: K,
    ): SettingsChoices<K> {
	...
	*/
    ...
    static async refreshSystem(
        system?: keyof ReturnType<typeof CITY_SETTINGS>["system"]["choices"],
    ) {
        if (!system) {
