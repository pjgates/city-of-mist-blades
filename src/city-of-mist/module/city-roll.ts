...
export class CityRoll {
    ...
    #prepareModifiers() {
        ...
        const allModifiers = this.#selectedList.filter((x) => {
            ...
            if (tag != null) {
                if (tag.isBurned())
                    console.log(
                        `Excluding ${tag.name}, value: ${tag.system.burned}`,
                    );
                return !tag.isBurned();
            } else return true;
        });
        ...
        if (!options.noTags) {
            tags = allModifiers.filter(
                (x) =>
                    x.type == "tag" &&
                    (
                        CityHelpers.getOwner(x.ownerId, x.tokenId) as CityActor
                    ).getTag(x.tagId), //filter out deleted tags
            );
        ...
        if (!options.noStatus) {
            ...
            const max = pstatus.reduce(
                (acc, x) => Math.max(acc, x.amount),
                -Infinity,
            );
            const min = nstatus.reduce(
                (acc, x) => Math.min(acc, x.amount),
                Infinity,
            );
        ...
        let modifiers = tags.concat(usedStatus).concat(helpHurt).concat(mods);
        if (options.themeType) {
            const blazetheme = options.BlazeThemeId
                ? actor.getTheme(options.BlazeThemeId)
                : undefined;
            modifiers.push({
                id: options.themeType,
                name:
                    localize(THEME_TYPES[options.themeType]) +
                    (blazetheme ? ` (${blazetheme.getDisplayedName()})` : ""),
	...
	*/
    ...
    static getRollPower(
        ...
        const base_power = validModifiers.reduce(
            (acc: number, x) => acc + x.amount,
            0,
        );
        const cap = validModifiers.some(
            (x) => x.subtype == "weakness" && x.amount < 0,
        )
            ? weaknessCap
            : 999;
    ...
    static getPower(
        ...
        if (CityHelpers.altPowerEnabled()) return this.getAltPower(rollOptions);
    ...
    static getRollStatus(
        ...
        if (CitySettings.get("autoFail_autoSuccess")) {
            if (roll.total == 12)
                return options.dynamiteAllowed && total >= 12
                    ? "Dynamite"
                    : "Success";
            if (roll.total == 2) return "Failure";
        }
    ...
    static calculateGritPenalty(standardPower: number) {
        ...
        if (standardPower >= 7) return -(standardPower - 4);
        else if (standardPower >= 4) return -(standardPower - 3);
        else if (standardPower == 3) return -(standardPower - 2);
        else return 0;
    }
    ...
    async #handleBurnTags() {
        ...
        if (!CitySettings.burnTemporaryTags()) return;
    ...
    async #handleWeakness() {
        ...
        for (const { ownerId, amount, tagId, tokenId } of tags) {
            ...
            if (
                tag.system.subtype == "weakness" &&
                amount < 0 &&
                CitySettings.isAutoWeakness()
            ) {
                await (
                    CityHelpers.getOwner(ownerId!) as CityActor
                )?.grantAttentionForWeaknessTag(tag.id);
            }
        }
    }
    ...
    async #deleteTempStatuses() {
        ...
        for (const { ownerId, id, tokenId } of statuses) {
            ...
            if (!status) throw new Error("Couldn't find status");
    ...
    static async verifyRequiredInfo(_move_id: string, actor: CityActor) {
        const relevantImprovements = actor
            .getImprovements()
            .filter((imp) => imp.hasEffectClass(`THEME_DYN_SELECT`));
    ...
    static addCreatedTag(
        options: MistRoll["options"],
        data: Omit<CreatedTagData, "type" | "options">,
    ) {
        if (data.name) {
    ...
    static async onCreateTag(
        chatMsg: MistChatMessage,
    ...
    static addCreatedStatus(
        options: MistRoll["options"],
        data: Omit<CreatedStatusData, "type" | "options">,
    ) {
        if (data.name) {
            options.createdItems.push({
                type: "status",
    ...
    static async onMEEffectButton(
        event: JQuery.ClickEvent,
        chatMsg: MistChatMessage,
    ): Promise<void> {
        const actionType = HTMLTools.getClosestData(
            event,
            "actionType",
        ) as MistEffect;
        const allowables = MIST_ENGINE_EFFECTS_OBJ[actionType];
        const otherArray = allowables.other.map((x) =>
            Object.fromEntries([
                [x, `MistEngine.rollEffects.effect.extra.${x}.name`],
            ]),
        );
        const allowableCreations = (
            allowables.status.length > 0
                ? [
                      { status: "CityOfMist.terms.status" } as Record<
                          string,
                          string
                      >,
                  ]
            ...
            .concat(
                allowables.tag.length ? [{ tag: "CityOfMist.terms.tag" }] : [],
            )
            .concat(otherArray);
        ...
        if (allowableCreations.length > 1) {
            ...
            result = await HTMLTools.singleChoiceBox(
                choiceData,
                "select Effect",
            );
        } else {
    ...
    static async deleteCreatedItem(
        ev: JQuery.ClickEvent,
        chatMsg: MistChatMessage,
    ): Promise<void> {
        const createdItems = chatMsg.rolls[0].options.createdItems;
        const index = Number(
            HTMLTools.getClosestDataNT(ev, "createdIndex", undefined),
        );
    ...
    static async deleteExtraFeat(
        ev: JQuery.ClickEvent,
        chatMsg: MistChatMessage,
    ): Promise<void> {
        const extraEffects = chatMsg.rolls[0].options.extraFeats;
        const index = Number(
            HTMLTools.getClosestDataNT(ev, "featIndex", undefined),
        );
        if (index == undefined) return;
    ...
    static createTagHtml(
        name: string,
        options: MistRoll["options"],
        creationOptions: TagCreationOptions = {},
    ): string {
    ...
    static createStatusHtml(
        name: string,
        options: MistRoll["options"],
        creationOptions: StatusCreationOptions,
    ): string {
        creationOptions.createdBy = this.getCreatorTags(options);
        return DragAndDrop.htmlDraggableStatus(name, creationOptions);
    }

    static statusOrTagHtmlFromRollData(
        options: MistRoll["options"],
        data: MistRoll["options"]["createdItems"][number],
    ): string {
        switch (data.type) {
            case "status":
    ...
    static getCreatorTags(
        options: MistRoll["options"],
    ): Tag["system"]["createdBy"] {
        const activeTags = options.tags.filter(
            (x) => !x.strikeout && x.amount > 0,
        );
            ...
            .map((tag) => ({
                ...
                actor: {
                    ...
                    token: tag.tokenId
                        ? {
                              tokenId: tag.tokenId,
                              scene: game.scenes.find((sc) =>
                                  sc.tokens.has(tag.tokenId!),
                              )!.id,
                          }
                        : undefined,
                },
    ...
    static async _strikeoutModifierToggle(event: Event): Promise<void> {
        ...
        message!.rolls.forEach((roll) => {
            const modifier = (roll.options.modifiers as RollModifier[]).find(
                (x) => x.id == modifierId,
            );
            if (modifier) {
    ...
    static async _checkOption(event: Event) {
        ...
        let moveList =
            message.getFlag<typeof moveListRaw>(
                "city-of-mist",
                "checkedOptions",
            ) ?? moveListRaw;
        ...
        if (!item) {
            ...
            if (!item) throw new Error(`Couldn't find ${listitem}`);
        }
        ...
        $(event.target!)
            .closest(".move-list")
            .find(".roll-selector-checkbox:checked")
            .each(function () {
    ...
    static createStoryTagHandlers(msg: MistChatMessage, html: JQuery) {
        ...
        if (!options) return;
        html.find(".city-roll .create-story-tag").on("click", (_ev) =>
            this.onCreateTag(msg),
        );
        html.find(".city-roll .create-status").on("click", (_ev) =>
            this.onCreateStatus(msg),
        );
        html.find(".city-roll .delete-created-item").on("click", (ev) =>
            this.deleteCreatedItem(ev, msg),
        );
        html.find(".city-roll .delete-extra-feat").on("click", (ev) =>
            this.deleteExtraFeat(ev, msg),
        );
        html.find(".city-roll .me-effects .me-effect").on("click", (ev) =>
            this.onMEEffectButton(ev, msg),
        );
        html.find(".city-roll .show-effects-list").on("click", (ev) =>
            this.showEffectsList(ev, msg),
        );
    }
} //end of class
